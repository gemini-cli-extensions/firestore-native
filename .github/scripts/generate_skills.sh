...
# Firestore skill names do not follow a "<prefix>-<toolset>" convention, so the
# skill name is configured explicitly alongside its upstream toolset.
SKILLS=(
  "firestore-data"
  "data"
  ...
  "firestore-security"
  "security"
  "Manage and validate Firestore security rules. Use to retrieve the active ruleset and check rule source for syntax and validation errors."
)
...
RAW_URL="https://raw.githubusercontent.com/googleapis/mcp-toolbox/v${VERSION}/internal/prebuiltconfigs/tools/firestore.yaml"
...
generate_skill() {
  ...
  npx "@toolbox-sdk/server@${VERSION}" --prebuilt firestore skills-generate \
    --name "$SKILL_NAME" \
